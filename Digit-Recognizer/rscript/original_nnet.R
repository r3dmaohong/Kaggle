...
vexplained <- cbind(c(1:784),vexplained,cumsum(vexplained[,1])) #cumsum() function returns the cumulative sums.
...
##class.ind(x)
Y <- class.ind(Y) #tranfer to a 1,0 matrix
print(X[1:5,1:5])
print(Y[1:5,])
...
finalseed <- 150
set.seed(finalseed)
model_final <- nnet(Xfinal,Y,size=150,softmax=TRUE,maxit=2000,MaxNWts = 80000)
...
write.csv(finalprediction, "output/nnet_predictions.csv", row.names=FALSE)
